                                        ...
                                        @foreach($col as $v)
                                            @if($v['index'])
                                                @php
                                                    switch($v['type']){
                                                        case 'date':
                                                            echo str_replace('col_name', $v['col'], '<td>@{{ date("Y-m-d",strtotime($v->col_name)) }}</td>');
                                                            break;
                                                        case 'textarea':
                                                            echo str_replace('col_name', $v['col'], '<td>@{{ mb_substr($v->col_name, 0, 50) }}</td>');
                                                            break;
                                                        case 'image':
                                                            echo str_replace('col_name', $v['col'], '<td><div style="max-height: 100px;max-width: 100px;"><img src="@{{ asset(is_null($v->col_name) ? \'public/images/default.png\':(\'storage/app/\'.$v->col_name )) }}"  alt="" class="img-responsive" style="max-height: 50px;"></div></td>');
                                                            break;
                                                        case 'select':
                                                            if($v['from_table'] && $v['f_name'])
                                                                echo str_replace('col_name', $v['from_table'].'_'.$v['f_name'].'->'.$v['f_name'], '<td>@{{ $v->col_name }}</td>');
                                                            else
                                                        ...
                                                        case 'checkbox':
                                                            ...
                                                            echo '$arr = [];';
                                                            echo '$arr = [';
                                                            foreach($v['default'] as $defKey=>$defValue){
                                                                echo "'$defKey'=>'$defValue',";
                                                            }
                                                            echo ']';
